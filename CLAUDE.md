...
| `quiz.js` | The warm-up quiz: draws questions from the study's pool, checks answers, builds the print worksheet |
...
- Set `hidden: true` on a studies.yml entry to omit it from the landing page, the navbar Studies menu, the breadcrumb study dropdown, and the settings page's study lists (use while a study is under construction). The landing page and breadcrumb filter hidden studies only in production builds — the dev server (`bin/bridgetown start`) shows them in both, the landing-page card carrying a 🚧 in-development marker (`.study-card-dev-badge`) on both sides of its title — an emoji rather than colored text so it stays visible in both themes; the leading one carries `aria-label="In development"` and the trailing one is `aria-hidden` so screen readers say it once — and even in production the breadcrumb still lists a hidden study on its own pages, so their navigation stays coherent. The navbar and settings lists filter hidden studies in every environment. Omitting the key or setting it to `false` shows the study. Hidden studies' pages are still built and reachable by direct URL — hidden means unlinked, not unpublished. Note that only the deploy workflow (`.github/workflows/deploy.yml`) sets `BRIDGETOWN_ENV=production`; a local `bin/bridgetown deploy` builds in development unless you set it yourself.
- The warm-up quiz is per-study: `src/{slug}/quiz.erb` (page at `/{slug}/quiz/`) renders `_partials/_quiz.erb`, and `quiz.js` builds the views from `src/_data/{slug}/quiz.yml` — a `weeks:` map of question pools, filed under the week each question's reading belongs to. `?week=N` quizzes on the weeks just before N; no params offers a quiz on the weeks just read (from progress) plus a week picker. Two reader settings in `bst_settings` shape every draw — `quizLength` (5–20 questions, default 10) and `quizLookback` (1–6 weeks before the week being started, default 4) — clamped in both `settings.js` and `quiz.js`, edited in the Settings page's Warm-Up Quiz section, which carries `data-feature="quiz"` and so hides with the toggle. The "Remember?" line on weekly overviews (rendered by `page.erb`, like the timeline's "When am I?" line) appears only when an earlier week has questions in that study's `quiz.yml`, and carries `data-feature="quiz"` so the global quiz toggle in settings controls it (opt-in, default off, like group and discussions — note the `!== true` FOUC pattern in `_head.erb`; the toggle hides the link, never the page). Scores are never graded or stored — the result exists only on screen, for the moment; every reveal names the answer in words (never color alone) and links back to the day page it came from. Printing a running quiz yields a worksheet: `quiz.js` builds the whole draw (numbered questions, checkbox choices, same shuffled order as on screen) into `#quiz-print`, followed by an Answer Key that always starts on its own page (answer, reference, week, and day per question), and the print stylesheet swaps it all in for the one-at-a-time view, keeping each question whole on a page; the week picker still prints as itself. The sheet is a one-column table because that is the repetition mechanism browsers support in print: its thead ("Warm-Up Quiz · Starting week N…") and tfoot (site name · print date) repeat as a running header and footer on every printed page, one tbody row per question carries the keep-together rule, and the on-page h1 is hidden in print so page 1 isn't double-headed. Only ntc1y has a `quiz.yml` so far.
- The timeline is per-study: `src/{slug}/timeline.erb` (page at `/{slug}/timeline/`) renders `_partials/_timeline.erb` from `src/_data/{slug}/timeline.yml`. The "When am I?" line on weekly overviews (rendered by `page.erb`) appears only for studies whose data directory contains `timeline.yml`; its `weeks:` map drives the hidden `#week-NN` "you are here" markers that CSS `:target` reveals — no JavaScript involved. Only ntc1y has one so far; an ot1y timeline needs a different, non-linear treatment (see `timeline.md`).
- `page-speak.js` hides `svg` elements (and anything marked `data-nospeak`) while taking its `innerText` snapshot, so inline SVG labels — the maps, the timeline — are never read aloud; figcaptions and the timeline's text listing still are. Elements marked `data-speak-pause` (the quiz's answer choices, progress line, and reference line) get a period appended for the snapshot unless they already end in punctuation, so each is spoken as its own sentence instead of running together.
