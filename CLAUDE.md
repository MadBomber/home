...
- Set `hidden: true` on a studies.yml entry to omit it from the landing page, the navbar Studies menu, the breadcrumb study dropdown, and the settings page's study lists (use while a study is under construction). The breadcrumb still lists a hidden study on its own pages, so its navigation stays coherent while under construction. Omitting the key or setting it to `false` shows the study. Hidden studies' pages are still built and reachable by direct URL — hidden means unlinked, not unpublished.
