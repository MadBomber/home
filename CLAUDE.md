...
- `page-speak.js` hides `svg` elements (and anything marked `data-nospeak`) while taking its `innerText` snapshot, so inline SVG labels — the maps, the timeline — are never read aloud; figcaptions and the timeline's text listing still are.
