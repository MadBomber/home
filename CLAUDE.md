...
| `quiz.js` | The warm-up quiz: draws questions from the study's pool, checks answers, keeps a personal best |
...
- **`storage_prefix`** in each `study_config.yml` must be unique across studies. It scopes the per-study keys only — `<prefix>_journal`, `<prefix>_progress`, and `<prefix>_quiz`.
